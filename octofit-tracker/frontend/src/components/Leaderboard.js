...
function Leaderboard() {
  ...
  const filteredItems = entries.filter((item) => {
    const primaryText = `${item.username ?? item.user ?? ''}`.toLowerCase();
      ...
      <div className="card page-card">
        <div className="card-header bg-white d-flex flex-wrap gap-2 justify-content-between align-items-center">
          ...
          <div className="d-flex gap-2">
            <button className="btn btn-primary" type="button" onClick={fetchEntries}>
        ...
        <div className="card-body">
          <p className="mb-3">
            <a className="link-primary" href={endpoint} target="_blank" rel="noreferrer">Leaderboard API endpoint</a>
          </p>

          <form className="row g-2 mb-3" onSubmit={(event) => event.preventDefault()}>
            <div className="col-md-8">
              <label htmlFor="leaderboardSearch" className="form-label">Zoeken</label>
              <input
                id="leaderboardSearch"
                className="form-control"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Filter op username"
              />
            </div>
            ...
            <table className="table table-striped table-hover align-middle mb-0">
              ...
              <tbody>
                {!filteredItems.length && (
                  <tr>
                    <td colSpan="5" className="text-center text-secondary">Geen leaderboard records gevonden.</td>
                  </tr>
                )}
                {filteredItems.map((item, index) => (
                  <tr key={item.id ?? index}>
                    <th scope="row">{index + 1}</th>
                    <td>{item.username ?? item.user ?? `Participant ${index + 1}`}</td>
