    ...
    <section className="container py-4">
      <div className="card page-card">
        ...
        <div className="card-body">
          <p className="mb-3">
            <a className="link-primary" href={endpoint} target="_blank" rel="noreferrer">Activities API endpoint</a>
          </p>

          <form className="row g-2 mb-3" onSubmit={(event) => event.preventDefault()}>
            <div className="col-md-8">
              <label htmlFor="activitiesSearch" className="form-label">Zoeken</label>
              <input
                id="activitiesSearch"
                className="form-control"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Filter op activity naam"
              />
            </div>
            ...
            <table className="table table-striped table-hover align-middle mb-0">
              ...
              <tbody>
                {!filteredItems.length && (
                  <tr>
                    <td colSpan="5" className="text-center text-secondary">Geen activiteiten gevonden.</td>
                  </tr>
                )}
